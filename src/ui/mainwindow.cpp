...
// Shown after a previous run aborted (SIGSEGV/SIGABRT…). Must use BlopModal —
// a raw full-window rgba backdrop with a free-floating frame left a permanent
// "black glass pane" on Android whenever the frame laid out off-screen.
void showCrashReportOverlay(QWidget *parent, const QString &report) {
  ...
  // Never stack on an existing modal/scrim.
  if (win->findChild<BlopModal *>(QString(), Qt::FindDirectChildrenOnly))
    return;
  const auto leftovers = win->findChildren<QWidget *>(
      QStringLiteral("BlopCrashBackdrop"), Qt::FindDirectChildrenOnly);
  ...
  form->setObjectName(QStringLiteral("BlopCrashReportForm"));
  auto *vlay = new QVBoxLayout(form);
  vlay->setContentsMargins(UiScale::dp(16), UiScale::dp(14), UiScale::dp(16),
                           UiScale::dp(14));
  vlay->setSpacing(UiScale::dp(8));

  auto *title = new QLabel(QObject::tr("Letzter Lauf ist abgestürzt"), form);
  title->setStyleSheet(BlopTheme::themed(
      QStringLiteral("color: #FF6B6B; font-size: 15px; font-weight: 700; "
  ...
  auto *btnCopy = new QPushButton(QObject::tr("Kopieren"), form);
  btnCopy->setStyleSheet(BlopTheme::secondaryButtonQss());
  auto *btnMail = new QPushButton(QObject::tr("Per Mail senden"), form);
  btnMail->setStyleSheet(BlopTheme::secondaryButtonQss());
  auto *btnDismiss = new QPushButton(QObject::tr("Verwerfen"), form);
  btnDismiss->setStyleSheet(BlopTheme::primaryButtonQss());
  ...
  BlopModal *modal = BlopModal::present(win, form, BlopModal::Mode::Card,
                                        QObject::tr("Absturzbericht"));
  if (!modal) {
    form->deleteLater();
    return;
  }
#ifdef Q_OS_ANDROID
  modal->setPreferredCardWidth(UiScale::dp(360));
#else
  modal->setPreferredCardWidth(UiScale::dp(520));
#endif

  QObject::connect(btnCopy, &QPushButton::clicked, form, [report]() {
    if (auto *cb = QGuiApplication::clipboard())
      cb->setText(report);
  });
  QObject::connect(btnMail, &QPushButton::clicked, form, [report]() {
    QUrl mailto;
  ...
  QObject::connect(btnDismiss, &QPushButton::clicked, modal, &BlopModal::dismiss);
  QObject::connect(modal, &BlopModal::dismissed, form, &QObject::deleteLater);
}
...
void syncAndroidHeaderGeometry(MainWindow *window) {
  ...
  const int btnW = narrowPhone
                       ...
                       : qBound(UiScale::dp(40), int(screenW * 0.11),
                                UiScale::dp(56));
  const int btnH =
      qBound(UiScale::dp(36), int(headerHeight * 0.75), UiScale::dp(44));
  ...
  headerLay->setSpacing(narrowPhone ? UiScale::dp(6) : UiScale::dp(8));
...
MainWindow::MainWindow(QWidget *parent)
  ...
  auto failOAuthFlow = [this](const QString &reason) {
    ...
    if (m_authNavigationLocked)
      setAndroidNativeLoginGateVisible(m_googleLoginInFlight);
#endif
  ...
  // previous_crash.txt at startup. Show via BlopModal Card (never a raw
  // full-window glass scrim). Delay past first paint + Study boot races.
...
void MainWindow::applyThemeRefresh() {
  ...
  if (m_libraryOrgBar)
    m_libraryOrgBar->setAccentColor(m_currentAccentColor);
  if (btnOverviewMenu)
    btnOverviewMenu->setIcon(
        createModernIcon(QStringLiteral("menu"), BlopTheme::textPrimary()));
  if (btnBackOverview)
    btnBackOverview->setIcon(
        createModernIcon(QStringLiteral("arrow_left"), BlopTheme::textPrimary()));
  if (btnStripMenu)
...
void MainWindow::ensureAndroidNativeLoginGate() {
  ...
  auto *sub = new QLabel(
      QStringLiteral("Google-Anmeldung"),
      gate);
...
void MainWindow::setupTitleBar() {
  ...
  m_btnEditorNoteOverflow->setIcon(
      createModernIcon(QStringLiteral("more_pill"), BlopTheme::textSecondary()));
  ...
  m_btnTitleBarPageManager->setIcon(createModernIcon(
      QStringLiteral("pages_pill"), BlopTheme::textSecondary()));
  {
...
void MainWindow::applyTheme() {
  ...
  // Overview: library chrome. Density follows the device — phone stays
  // touch-sized, desktop keeps the quieter Drawboard action bar.
  if (m_overviewContainer) {
    const bool phoneLibrary = UiScale::isAndroidPhoneUi(this);
    const QString titleSize = phoneLibrary ? QStringLiteral("22")
#ifdef Q_OS_ANDROID
                                           : QStringLiteral("24");
#else
                                           : QStringLiteral("26");
    ...
    QString overviewQss = QStringLiteral(
        ...
        "  color: %2; font-size: %3px; font-weight: 700;"
        ...
        "  background-color: %8; color: #FFFFFF; border-radius: 10px;"
        "  padding: 0 14px; font-weight: 700; font-size: 13px;"
        "  border: none; min-height: %9px; max-height: %9px;"
        "}");
    ...
    if (m_lblLibraryTitle)
      m_lblLibraryTitle->setStyleSheet(QString());
    if (m_lblLibrarySubtitle)
      m_lblLibrarySubtitle->setStyleSheet(QString());
    if (m_overviewSearchBar)
      m_overviewSearchBar->setStyleSheet(QString());
    if (auto *folderBtn = m_overviewContainer->findChild<QPushButton *>(
            QStringLiteral("overviewBtnNewFolder")))
  ...
  if (m_lblSidebarUser) {
    m_lblSidebarUser->setStyleSheet(QStringLiteral(
...
void MainWindow::setupUi() {
  ...
  const int androidHeaderButtonH =
      qBound(UiScale::dp(36), int(androidHeaderHeight * 0.75), UiScale::dp(44));
  const int androidCompactPillH =
      qBound(UiScale::dp(36), int(androidHeaderHeight * 0.72), UiScale::dp(44));
...
#ifdef Q_OS_ANDROID
  const bool phoneHeader = UiScale::isAndroidPhoneUi(this);
  headerLayout->setContentsMargins(0, UiScale::dp(phoneHeader ? 4 : 8),
                                   0, UiScale::dp(phoneHeader ? 8 : 12));
  headerLayout->setSpacing(UiScale::dp(phoneHeader ? 8 : 10));
  ...
  m_lblLibraryTitle->setStyleSheet(BlopTheme::themed(
      QStringLiteral("color: #F4F5FB; font-size: %1px; font-weight: 700;"
                     " letter-spacing: -0.3px; background: transparent;")
          .arg(phoneHeader ? 22 : 24)));
  ...
  if (phoneHeader) {
    ...
    btnNewFolder->setFixedSize(UiScale::dp(44), UiScale::dp(44));
    ...
    btnNewFolder->setText(QStringLiteral("Ordner"));
    btnNewFolder->setFixedHeight(UiScale::dp(44));
    btnNewFolder->setStyleSheet(
        "QPushButton {"
  ...
  m_emptyStateHost = new QWidget(libraryMain);
  m_emptyStateHost->setObjectName(QStringLiteral("LibraryEmptyState"));
  m_emptyStateHost->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
  ...
  connect(m_btnEmptyCta, &QPushButton::clicked, this, [this]() {
    ...
    const bool searchFiltered =
        m_overviewSearchBar && !m_overviewSearchBar->text().trimmed().isEmpty();
    const bool tagFiltered =
        m_libraryTagsPanel && !m_libraryTagsPanel->selectedTags().isEmpty();
    if (smartFiltered || searchFiltered || tagFiltered) {
      if (m_libraryOrgBar)
        m_libraryOrgBar->setSmartView(LibraryOrgBar::SmartView::All);
      if (m_overviewSearchBar)
        m_overviewSearchBar->clear();
      if (m_libraryTagsPanel)
        m_libraryTagsPanel->clearSelection();
      applyLibraryFilters();
      return;
    }
    onNewPage();
  ...
  m_btnNoteHeaderLayout = new ModernButton(m_noteHeader);
  m_btnNoteHeaderLayout->setObjectName(QStringLiteral("NoteHeaderLayoutBtn"));
  m_btnNoteHeaderLayout->setFixedSize(UiScale::dp(32), UiScale::dp(32));
  m_btnNoteHeaderLayout->setToolTip(QStringLiteral("Seitenlayout & Hintergrund"));
  connect(m_btnNoteHeaderLayout, &QAbstractButton::clicked, this, [this]() {
  ...
  noteHeaderLayout->addWidget(m_btnNoteHeaderLayout);
  styleNoteHeaderChrome();
...
void MainWindow::applyAndroidTabStyles(int index) {
  ...
  const int pillMaxW = narrow ? UiScale::dp(102) : UiScale::dp(120);
...
void MainWindow::onModeChanged(int index) {
  ...
  if (mainStackIdx == 0) {
    ...
    // Clear any leftover C++ glass panes that outlived a Study crash.
    const auto crashPanes = findChildren<QWidget *>(
        QStringLiteral("BlopCrashBackdrop"), Qt::FindChildrenRecursively);
    for (QWidget *w : crashPanes) {
      if (w)
        w->deleteLater();
    }
    if (m_androidSidebarScrim && m_androidSidebarScrim->isVisible() &&
        (!m_sidebarContainer || m_sidebarContainer->width() < UiScale::dp(8))) {
      m_androidSidebarScrim->hide();
    }
  }
  bool deferStudyStackSwitch = false;
  if (mainStackIdx == 1 && m_studyQQuickView
...
void MainWindow::setupSidebar() {
  ...
  // Select Bibliothek by default (row after Favoriten).
  ...
  midLay->addWidget(notesHeader);
  m_sidebarNotesList = new QListWidget(mid);
  m_sidebarNotesList->setObjectName(QStringLiteral("SidebarNotesList"));
  m_sidebarNotesList->setFrameShape(QFrame::NoFrame);
  m_sidebarNotesList->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
  m_sidebarNotesList->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
  m_sidebarNotesList->setSpacing(1);
  m_sidebarNotesList->setStyleSheet(QStringLiteral(
      "QListWidget#SidebarNotesList { background: transparent; border: none; outline: 0; }"
      "QListWidget#SidebarNotesList::item {"
      "  color: #D5D8E0; padding: 6px 10px; border-radius: 6px;"
      "}"
      "QListWidget#SidebarNotesList::item:hover { background: rgba(255,255,255,0.06); }"
      "QListWidget#SidebarNotesList::item:selected { background: rgba(255,255,255,0.10); }"));
  connect(m_sidebarNotesList, &QListWidget::itemClicked, this,
...
void MainWindow::refreshSidebarNotesList() {
#ifndef Q_OS_ANDROID
  if (!m_sidebarNotesList)
    return;
  m_sidebarNotesList->clear();
  QStringList paths = LibraryOrgStore::recentPaths(8);
...
void MainWindow::updateSidebarBadges() {
  ...
  if (m_emptyStateHost && m_fileListView && m_fileModel) {
    ...
    if (visible == 0) {
      ...
      m_fileListView->hide();
    } else if (m_emptyStateHost->isVisible()) {
      m_emptyStateHost->hide();
      m_fileListView->show();
    }
  }
...
void MainWindow::onNavItemClicked(QListWidgetItem *item) {
  ...
  if (item->text() == QStringLiteral("Papierkorb")) {
    const QString trash = QDir(m_rootPath).filePath(QStringLiteral(".Papierkorb"));
    QDir().mkpath(trash);
    if (m_libraryOrgBar)
      m_libraryOrgBar->setSmartView(LibraryOrgBar::SmartView::All);
...
void MainWindow::styleNoteHeaderChrome() {
  if (!m_noteHeader)
    return;
  m_noteHeader->setStyleSheet(
      QStringLiteral("QWidget#NoteHeader {"
                     "  background: %1;"
  ...
  if (m_lblNoteHeaderTitle) {
    m_lblNoteHeaderTitle->setStyleSheet(
        QStringLiteral("color: %1; font-size: 14px; font-weight: 700;"
                       " letter-spacing: -0.2px; background: transparent;")
  ...
  if (m_btnNoteHeaderLayout) {
    m_btnNoteHeaderLayout->setIcon(
        createModernIcon(QStringLiteral("palette"), NoteChrome::textSecondary()));
    m_btnNoteHeaderLayout->setStyleSheet(
        QStringLiteral("QToolButton { background: %1; border: 1px solid %2;"
                       "  border-radius: 10px; }"
          ...
          "QPushButton:hover { background: %4; border-color: %5; }")
                                             .arg(BlopTheme::surfaceMuted().name(
                                                  ...
                                                  BlopTheme::borderSubtle().name(
                                                      QColor::HexArgb),
                                                  plusFg.name(QColor::HexRgb),
                                                  BlopTheme::surfaceElevated().name(
                                                      QColor::HexRgb),

...
def build_vector_store(text_chunks):
    ...
    try:
        ...
        key_status = "Found" if "GOOGLE_API_KEY" in os.environ else "Missing"
        st.error(f"Vector Store Error (FAISS): {e} | Key: {key_status}")
        return None, []
    batch_size = 1 
...
def render_workspace_content(username, folder_id):
    ...
    # --- VIEW: Interaktives Lernen ---
        ...
        st.header("🧠 Interaktives Lernen")
        
        tab_cards, tab_quiz = st.tabs(["🗂️ Karteikarten", "❓ Quiz"])
        ...
        with tab_cards:
            ...
            with st.expander("✨ Neue Karten generieren", expanded=not cards):
                if st.button("Karten aus Dokumenten erstellen (AI)", key="btn_gen_cards"):
                    if "text_chunks" in st.session_state and st.session_state.text_chunks:
        ...
        # --- TAB: QUIZ ---
        with tab_quiz:
            st.subheader("Wissens-Check")
            if st.button("Neues Quiz generieren", key="btn_gen_quiz"):
                with st.spinner("..."):
                    ...
                    with st.spinner("KI erstellt Zusammenfassung... (kann 30-60s dauern)"):
                        try:
                            ...
                            all_text = "\n".join([
                                ...
                                for c in st.session_state.text_chunks[:30]
                            ])

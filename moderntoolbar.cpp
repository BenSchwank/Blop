...
void ToolbarBtn::setPressedState(bool pressed) { m_pressed = pressed; update(); }

// WICHTIG: Events ignorieren, damit ModernToolbar sie bekommt!
void ToolbarBtn::mousePressEvent(QMouseEvent* e) { e->ignore(); }
void ToolbarBtn::mouseMoveEvent(QMouseEvent* e) { e->ignore(); }
void ToolbarBtn::mouseReleaseEvent(QMouseEvent* e) { e->ignore(); }
...
void ToolbarBtn::paintEvent(QPaintEvent*) {
    ...
    // Hintergrund-Logik
    if (m_pressed) {
        // Dunklerer Hintergrund beim Drücken
        p.setBrush(QColor(0x5E5CE6).darker(130)); p.setPen(Qt::NoPen);
...
ModernToolbar::ModernToolbar(QWidget *parent) : QWidget(parent) {
    ...
    // KEINE Connects mehr hier für clicked(), das machen wir manuell in triggerButtonAction

    setStyle(Normal);
    setToolMode(ToolMode::Pen);
}

void ModernToolbar::triggerButtonAction(ToolbarBtn* btn) {
    ...
    if (btn == btnPen) handleToolClick(ToolMode::Pen);
    else if (btn == btnEraser) handleToolClick(ToolMode::Eraser);
    else if (btn == btnLasso) handleToolClick(ToolMode::Lasso);
    else if (btn == btnUndo) emit undoRequested();
    else if (btn == btnRedo) emit redoRequested();
}

void ModernToolbar::showEvent(QShowEvent*) {
    updateLayout();
    if (m_style == Radial) constrainToParent();
}
...
void ModernToolbar::setStyle(Style style) {
    ...
    if (m_style == Normal) {
        ...
        if (m_orientation == Vertical) resize(55, 310);
        else resize(310, 55);
    } else {
...
void ModernToolbar::checkOrientation(const QPoint& globalPos) {
    ...
    int distLeft = local.x();
    int distRight = w - local.x();
    int distTop = local.y();
    int distBottom = h - local.y();

    int minDistVert = qMin(distTop, distBottom);
    int minDistSide = qMin(distLeft, distRight);

    if (minDistVert < 100 && minDistVert < minDistSide) {
...
void ModernToolbar::setOrientation(Orientation o) {
    ...
    anim->setEndValue(QSize(s.height(), s.width()));
...
int ModernToolbar::calculateMinLength() {
    int btnS = 40 * m_scale;
    if (!m_buttons.isEmpty()) btnS = m_buttons[0]->width();
    int dragH = 50;
    int handleH = 30;
    int minGap = 5;
...
void ModernToolbar::updateLayout(bool animate) {
    if (m_style == Normal) {
        ...
        int breadth = (m_orientation == Vertical) ? w : h;
...
void ModernToolbar::paintEvent(QPaintEvent *) {
    ...
    if (m_style == Normal) {
        ...
        if (m_orientation == Vertical) {
            p.drawRoundedRect(w/2 - 12, 25, 24, 4, 2, 2);
            int knobSize = 25;
            p.setBrush(QColor(50,50,50, 255)); p.setPen(QPen(QColor(80,80,80), 1));
            p.drawEllipse(w - knobSize + 5, h - knobSize + 5, knobSize, knobSize);
            p.setPen(QPen(Qt::white, 2));
            int kx = w - knobSize + 5; int ky = h - knobSize + 5;
            p.drawLine(kx+8, ky+8, kx+16, ky+16);
        } else {
            p.drawRoundedRect(25, h/2 - 12, 4, 24, 2, 2);
            int knobSize = 25;
            p.setBrush(QColor(50,50,50, 255)); p.setPen(QPen(QColor(80,80,80), 1));
            p.drawEllipse(w - knobSize + 5, h - knobSize + 5, knobSize, knobSize);
            p.setPen(QPen(Qt::white, 2));
...
void ModernToolbar::mousePressEvent(QMouseEvent *e) {
    if (e->button() == Qt::LeftButton) {
        ...
        if (m_style == Normal) {
            ...
            // Klick auf Button prüfen
            for(auto* b : m_buttons) {
                if(b->geometry().contains(e->pos()) && b->isVisible()) {
                    m_highlightedBtn = b;
                    m_highlightedBtn->setPressedState(true);
                    m_pressPos = e->pos();
                    return;
                }
            }
        ...
        // Klick außerhalb des Menüs -> Weiterreichen an Canvas!
        if (dist > visualR) {
            e->ignore();
            return;
        }
        ...
        for(auto* b : m_buttons) {
            if(b->geometry().contains(e->pos()) && b->isVisible()) {
                m_highlightedBtn = b;
                m_highlightedBtn->setPressedState(true); // Visuell drücken
                m_pressPos = e->pos();
                return;
            }
        }

        // Scroll Start (Ring) - Nur bei HalfEdge
        if (m_radialType == HalfEdge && dist > dragR && dist <= rMain) {
            m_isScrolling = true;
            m_lastInteractionAngle = std::atan2(e->pos().y() - cy, e->pos().x() - cx) * 180.0 / M_PI;
            return;
        }

        // Settings Interaktion (Ringe)
        if (m_settingsState != SettingsState::Closed) {
...
void ModernToolbar::mouseMoveEvent(QMouseEvent *e) {
    // Wenn wir einen Button drücken, aber uns bewegen -> Scrollen/Draggen statt Klicken
    if (m_highlightedBtn) {
        int dist = (e->pos() - m_pressPos).manhattanLength();
        if (dist > 10) { // Schwellenwert
            m_highlightedBtn->setPressedState(false);
            m_highlightedBtn = nullptr;

            // Jetzt entscheiden: Scrollen oder Draggen?
            // Bei HalfEdge im Ring -> Scrollen
            int cx = (m_style == Radial && m_radialType == HalfEdge) ? (m_isDockedLeft ? 0 : width()) : width()/2;
            int cy = height()/2;
            int dx = e->pos().x() - cx; int dy = e->pos().y() - cy;
            double d = std::sqrt(dx*dx + dy*dy);

            if (m_style == Radial && m_radialType == HalfEdge && d > 45*m_scale) {
                m_isScrolling = true;
                m_lastInteractionAngle = std::atan2(e->pos().y() - cy, e->pos().x() - cx) * 180.0 / M_PI;
            } else {
                // Sonst Drag (oder ignorieren)
                // Hier vereinfacht: Wir machen nix, Nutzer muss neu ansetzen zum Scrollen
            }
        }
        return;
    }
    ...
    if (m_isResizing && m_style == Normal) {
        ...
        if (m_orientation == Vertical) {
            ...
            if (newW < 50) newW = 50; if(newW > 100) newW = 100;
        } else {
            if (newW < minL) newW = minL;
            if (newH < 50) newH = 50; if(newH > 100) newH = 100;
        }
        resize(newW, newH);
        constrainToParent();
        return;
    ...
    if (m_isScrolling) {
        ...
        int cy = height() / 2;
        double currentAngle = std::atan2(e->pos().y() - cy, e->pos().x() - cx) * 180.0 / M_PI;

        double delta = currentAngle - m_lastInteractionAngle;
        ...
        m_lastInteractionAngle = currentAngle;
        updateLayout();
        return;
    }

    if (m_style == Normal) {
        if (e->pos().x() > width() - 30 && e->pos().y() > height() - 30) setCursor(Qt::SizeFDiagCursor);
...
void ModernToolbar::mouseReleaseEvent(QMouseEvent *) {
    ...
    // Button Klick auslösen, falls wir nicht gescrollt haben
    if (m_highlightedBtn) {
        triggerButtonAction(m_highlightedBtn);
        m_highlightedBtn->setPressedState(false);
        m_highlightedBtn = nullptr;
    }
...
void ModernToolbar::showVerticalPopup() {
    ...
    if (mode_ == ToolMode::Pen) {
        lay->addWidget(new QLabel("Stift"));
...
void ModernToolbar::handleRadialSettingsClick(const QPoint& pos, int cx, int cy, int, int) {
    double angle = std::atan2(-(pos.y() - cy), (pos.x() - cx)) * 180.0 / M_PI;
        ...
        if (m_settingsState == SettingsState::ColorSelect) {
            ...
            if (idx >= 0 && idx < count) {
                ...
                if (m_config.penColor == c) {
                    ...
                    m_config.penColor = c; emit penConfigChanged(c, m_config.penWidth);
                }
                update();
            }
        } else if (m_settingsState == SettingsState::SizeSelect) {

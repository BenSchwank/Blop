...
#include <QRegion>
    ...
    void ModernToolbar::constrainToParent() {
        ...
        // FIX: Padding berücksichtigen (Widget ist größer als sichtbarer Kreis)
        int padding = 0;
        if (m_style == Radial) {
        ...
        if (m_style == Radial) {
            if (m_radialType == HalfEdge && !m_isDragging) snapToEdge();
                ...
                if (!m_isPreview && parentWidget() && !m_isDragging) {
                    // Hier auch Padding beachten?
                    // Einfach constrainToParent aufrufen ist sicherer
                    constrainToParent();
                }
            }
            updateLayout(true);
    ...
    void ModernToolbar::mouseMoveEvent(QMouseEvent *e) {
        ...
        if (m_isDragging) {
            ...
            // FIX: Padding berechnen (wie in constrainToParent)
            ...
            if (m_style == Radial) {
                ...
                // B) ABREISSEN (Half -> Full)
                    ...
                    if (pullAway) {
                        ...
                        // Padding für vertikales Sliden berücksichtigen
            ...
            // Padding auch unten erlauben
            int maxY = parentH - height() + effectivePadding;

            // Padding auch oben erlauben (wenn Radial)

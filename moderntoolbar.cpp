...
void ModernToolbar::updateHitbox() { if (m_isDragging) return; if (m_style != Radial) { clearMask(); m_cachedMask = QRegion(); return; } int r = 95 * m_scale; if (m_settingsState != SettingsState::Closed) { if (m_settingsState == SettingsState::Main) r = 135 * m_scale; else r = 175 * m_scale; } int maskR = r + 4; QRegion newMask; if (m_radialType == FullCircle) { int cx = width() / 2; int cy = height() / 2; newMask = QRegion(cx - maskR, cy - maskR, 2 * maskR, 2 * maskR, QRegion::Ellipse); } else { int cy = height() / 2; int cx = m_isDockedLeft ? 0 : width(); QRegion circleReg(cx - maskR, cy - maskR, 2 * maskR, 2 * maskR, QRegion::Ellipse); QRegion widgetReg(rect()); newMask = circleReg.intersected(widgetReg); } if (newMask == m_cachedMask) return; if (m_settingsState == SettingsState::Closed) { clearMask(); repaint(); } setMask(newMask); m_cachedMask = newMask; }
...
void ModernToolbar::mousePressEvent(QMouseEvent *e) { if (e->button() == Qt::LeftButton) { m_pressedButton = nullptr; m_isScrolling = false; m_hasScrolled = false; m_isDragging = false; if (m_style == Radial) { m_pressedButton = getRadialButtonAt(e->pos()); } int dragR = 45 * m_scale; int cx = width()/2; int cy = height()/2; if (m_style == Radial && m_radialType == HalfEdge) cx = m_isDockedLeft ? 0 : width(); int dx = e->pos().x() - cx; int dy = e->pos().y() - cy; double dist = std::sqrt(dx*dx + dy*dy); if (m_style == Radial && m_radialType == HalfEdge) { if (dist > dragR) { m_dragStartAngle = std::atan2(-dy, dx) * 180.0 / 3.14159; m_scrollStartAngleVal = m_scrollAngle; m_isScrolling = true; } } if (m_style == Radial && m_radialType == FullCircle && !m_pressedButton) { int rMain = 95 * m_scale; int r1_out = 135 * m_scale; int r2_out = 175 * m_scale; if (m_settingsState != SettingsState::Closed) { if (dist > rMain && dist < r1_out) { handleRadialSettingsClick(e->pos(), width()/2, height()/2, rMain, r1_out); return; } if (dist > r1_out && dist < r2_out && m_settingsState != SettingsState::Main) { handleRadialSettingsClick(e->pos(), width()/2, height()/2, r1_out, r2_out); return; } } } if (m_isResizing) return; if (m_style == Normal && !m_isPreview) { int handleSize = 30; if (e->pos().x() > width() - handleSize && e->pos().y() > height() - handleSize) { m_isResizing = true; dragStartPos_ = e->pos(); resizeStartPos_ = e->pos(); startSize_ = size(); return; } } if (!m_isPreview && !m_pressedButton) { bool canDrag = false; if (m_style == Radial) { if (dist < dragR) canDrag = true; } else { if (m_orientation == Vertical && e->pos().y() < 50) canDrag = true; if (m_orientation == Horizontal && e->pos().x() < 50) canDrag = true; } if (canDrag) { m_isDragging = true; dragStartPos_ = e->pos(); m_isScrolling = false; m_dragOffset = e->globalPosition().toPoint() - mapToGlobal(QPoint(0,0)); clearMask(); return; } } } }
void ModernToolbar::handleRadialSettingsClick(const QPoint& pos, int cx, int cy, int, int) { double angle = std::atan2(-(pos.y() - cy), (pos.x() - cx)) * 180.0 / 3.14159; if (angle < 0) angle += 360.0; int dx = pos.x() - cx; int dy = pos.y() - cy; double dist = std::sqrt(dx*dx + dy*dy); bool isRing2 = (dist > 135 * m_scale); double startA = 0; double spanAngleDeg = 360; if (m_radialType == HalfEdge) { startA = m_isDockedLeft ? -90 : 90; spanAngleDeg = 180; } double currentA = angle; if (m_radialType == HalfEdge && m_isDockedLeft && currentA > 270) currentA -= 360; double relA = currentA - startA; while(relA < 0) relA += 360; if (m_radialType == HalfEdge && (relA < 0 || relA > spanAngleDeg)) return; if (!isRing2) { double midLimit = spanAngleDeg / 2.0; if (relA < midLimit) { if (m_settingsState == SettingsState::ColorSelect) m_settingsState = SettingsState::SizeSelect; else m_settingsState = SettingsState::ColorSelect; } else { if (m_settingsState == SettingsState::SizeSelect) m_settingsState = SettingsState::Closed; else m_settingsState = SettingsState::SizeSelect; } update(); updateHitbox(); } else { if (m_settingsState == SettingsState::ColorSelect) { int count = m_customColors.size(); double step = spanAngleDeg / (count + 1); int idx = (int)(relA / step) - 1; if (idx >= 0 && idx < count) { QColor c = m_customColors[idx]; if (m_config.penColor == c) { QColor newC = QColorDialog::getColor(c, this, "Farbe ändern"); if (newC.isValid()) { m_customColors[idx] = newC; m_config.penColor = newC; emit penConfigChanged(newC, m_config.penWidth); } } else { m_config.penColor = c; emit penConfigChanged(c, m_config.penWidth); } update(); } } else if (m_settingsState == SettingsState::SizeSelect) { int count = 5; double step = spanAngleDeg / (count + 1); int idx = (int)(relA / step) - 1; if (idx >= 0 && idx < count) { m_config.penWidth = (idx + 1) * 3 + 2; emit penConfigChanged(m_config.penColor, m_config.penWidth); update(); } } } }
